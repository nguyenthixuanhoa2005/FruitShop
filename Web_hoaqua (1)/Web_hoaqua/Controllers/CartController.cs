...
namespace Web_hoaqua.Controllers
{
    public class CartController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            // Người dùng đã đăng nhập: lấy giỏ hàng từ DB để đảm bảo luôn đúng dữ liệu mới nhất.
            if (!string.IsNullOrEmpty(userIdStr) && int.TryParse(userIdStr, out int userId))
            {
                cartItems = _context.CartItems
                     .Include(c => c.Product)
                     .Where(c => c.UserId == userId && c.Status == 1 && c.Product != null)
                     .ToList();

                // Đồng bộ lại Session để các luồng khác (badge, checkout...) dùng cùng trạng thái.
                var sessionCart = cartItems
                     .Where(c => c.ProductId.HasValue)
                     .ToDictionary(c => c.ProductId!.Value, c => c.Quantity ?? 0);
                HttpContext.Session.SetString("Cart", JsonSerializer.Serialize(sessionCart));
            }
            else
            {
                // Fallback nếu chưa đăng nhập (hoặc phiên không hợp lệ): dùng Session.
        ...
        [HttpPost] // có ý nghĩa dùng POST để tránh bị cache và đảm bảo an toàn khi gửi dữ liệu
        ...
        public async Task<IActionResult> CreateOrder([FromBody] OrderCheckoutRequest request)
        {
            try
            {
                ...
                // Sinh mã thanh toán duy nhất (6 ký tự: DH + 4 số/chữ ngẫu nhiên)
                string paymentRef = "DH" + Guid.NewGuid().ToString("N").Substring(0, 4).ToUpper();
                ...
                var payment = new Payment
                {
                    ...
                    TransactionId = request.PaymentMethod == "BANK_TRANSFER" ? paymentRef : null,
                    CreatedAt = DateTime.Now
                };
        ...
        [HttpPost]
        [IgnoreAntiforgeryToken] // Bỏ validate token vì đây là endpoint công khai để SePay gọi về
        public async Task<IActionResult> SePayWebhook([FromBody] JsonElement data)
        {
            try
            {
                // 1. Trích xuất nội dung và số tiền từ Webhook SePay
                ...
                // 2. Tìm kiếm theo mã duy nhất DHxxxx (Mã mới chúng ta vừa áp dụng)
                var match = System.Text.RegularExpressions.Regex.Match(content, @"DH[A-Z0-9]{4}", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                if (match.Success)
                {
                    string paymentRef = match.Value.ToUpper();
                    payment = await _context.Payments
                         .Include(p => p.Order)
                         .FirstOrDefaultAsync(p => p.TransactionId == paymentRef && p.Method == "BANK_TRANSFER");

                    if (payment != null) order = payment.Order;
                }

                // 3. Fallback: Nếu không tìm thấy mã DHxxxx, thử tìm theo mã ĐH-xxx (để hỗ trợ đơn hàng cũ)
                if (order == null)
                {
                ...
                // 4. Kiểm tra điều kiện và cập nhật trạng thái
                if (order != null && (order.Status == 1 || order.Status == null))
                {
                    // Chỉ xác nhận nếu số tiền khách chuyển >= số tiền đơn hàng
                    if (transferAmount >= (order.TotalAmount ?? 0))
                    {
                        ...
                        if (payment != null)
                        {
                            payment.Status = 4; // 4: Thành công
                                                // Lưu ID giao dịch của SePay vào TransactionId (ngăn cách với mã FR bằng dấu gạch đứng)
                            string sePayId = data.TryGetProperty("id", out var idElem) ? idElem.ToString() : "";
                            payment.TransactionId = $"{payment.TransactionId} | {sePayId}";
                        }
                ...
                return Ok(new { success = false, message = "Không tìm thấy đơn hàng hoặc trạng thái không hợp lệ" });
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Lỗi SePayWebhook: {ex.Message}");
                return BadRequest(new { error = ex.Message });
            }
        }
        // Hàm phụ để trích xuất OrderId từ nội dung chuyển khoản với nhiều định dạng khác nhau
