...
GREETING_FOLLOWUP_WORDS = {
    "ban", "shop", "oi", "nha", "a", "em", "anh", "chi", "ad", "admin"
}
PRODUCT_HINT_WORDS = {
...
def classify_intent(user_query: str) -> str:
    ...
    has_greeting_signal = is_greeting_query(user_query)

    if has_greeting_signal and not has_product_hint and not has_price:
        return "greeting"
...
def is_greeting_query(user_query: str) -> bool:
    ...
    token_set = set(tokens)
    greeting_tokens = {token for token in GREETING_WORDS if " " not in token}
    greeting_phrases = {phrase for phrase in GREETING_WORDS if " " in phrase}
    followup_tokens = {strip_accents(word) for word in GREETING_FOLLOWUP_WORDS}

    if any(phrase in normalized for phrase in greeting_phrases):
        return True

    if token_set & greeting_tokens:
        non_greeting_tokens = token_set - greeting_tokens - followup_tokens
        if len(non_greeting_tokens) <= 1:
            return True

    if len(tokens) <= 4 and normalized.startswith(("xin chao", "chao", "hello", "hi", "hey", "alo", "yo")):
        return True
...
def hybrid_retrieve(
    ...
    # Lấy thông tin lịch sử người dùng để cá nhân hóa
    ...
    if user_id:
        try:
            ...
            cursor = conn.cursor()
            # Sản phẩm đã mua
            cursor.execute("SELECT DISTINCT product_id FROM order_items oi JOIN orders o ON oi.order_id = o.id WHERE o.user_id = ?", (user_id,))
            user_history_ids = {int(row[0]) for row in cursor.fetchall() if row[0]}
            # Sản phẩm trong giỏ
            cursor.execute("SELECT DISTINCT product_id FROM cart_items WHERE user_id = ? AND status = 1", (user_id,))
            user_cart_ids = {int(row[0]) for row in cursor.fetchall() if row[0]}
            conn.close()
    ...
    for product in PRODUCTS_CACHE:
        ...
        for t in query_token_set:
            if token_matches_blob(t, blob):
                text_score += 2

        v_score = vector_scores.get(product["id"], 0.0)
        
        # Cá nhân hóa điểm số
        personalized_bonus = 0
        if product["id"] in user_cart_ids:
            personalized_bonus += 5 # Ưu tiên sản phẩm trong giỏ
        if product["id"] in user_history_ids:
            personalized_bonus += 3 # Ưu tiên sản phẩm đã từng mua

        total_score = (text_score * 0.4) + (v_score * 0.6) + personalized_bonus
        if total_score > 0:
            candidate = build_candidate_record(product, text_score, 0.0, v_score)
            candidate["score"] = total_score
            candidate["hybrid"] = total_score
...
def rerank_candidates(candidates, query_tokens, answer_mode: str, prefer_cheaper: bool = False):
    ...
    for item in candidates:
        ...
        base_score = item.get("hybrid", item.get("score", 0.0))
        item["rerank"] = base_score + boost
...
def resolve_compare_products(
    ...
    if len(candidates) < 2:
        ranked = hybrid_retrieve(
            ...
            top_k=6,
            user_id=None, # In resolve_compare_products we might not have session_state directly, or we can pass it as param
        )
        for item in ranked:
...
def build_general_chat_answer_with_llm(user_query: str, sales_coach_mode: bool = False, memory_context: str = ""):
    ...
    prompt = (
        ...
        "Cảm ơn và xin chào, nếu có bất kỳ thắc mắc xin liên hệ trực tiếp theo thông tin này:\n"
...
def load_products_cache(limit: int = 500):
    ...
    try:
        ...
        print(f"Successfully loaded {len(PRODUCTS_CACHE)} products into cache.")
...
async def chat_with_ai(request: Request):
    ...
    # Cập nhật user_id vào session_state
    ...
    if is_greeting_query(user_query_raw):
        return finalize_chat_response(session_state, {
            "answer": "Chào bạn. Bạn muốn mình tư vấn theo loại trái cây, tầm giá, hay mục đích dùng (ăn liền, ép nước, làm quà)?",
            "products": [],
            "answer_mode": "chat",
        })
